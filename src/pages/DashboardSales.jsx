...
import TotalOrders from "@/components/dashboardSales/TotalOrders";
import TotalProductsSold from "@/components/dashboardSales/TotalProductsSold";
import AvgOrder from "@/components/dashboardSales/AvgOrder";
import SalesOverTime from "@/components/dashboardSales/SalesOverTime";
import ProductSell from "@/components/dashboardSales/ProductSell";
import SalesList from "@/components/dashboardSales/SalesList";

// ฟังก์ชันตรวจสอบว่าเลือกช่วงเวลาไหน
    ...
    setDate((prevDate) => {
      ...
      switch (groupBy) {
        case "hour": // วันนี้ -> เพิ่ม/ลดวันเดียว
          newFrom = direction === "prev" ? subDays(prevDate.from, 1) : addDays(prevDate.from, 1);
          newTo = newFrom;
          break;
        
        case "day": 
          if (isSameDay(prevDate.from, startOfWeek(prevDate.from, { weekStartsOn: 1 }))) {
            // สัปดาห์ -> จันทร์-อาทิตย์
            newFrom = direction === "prev" ? startOfWeek(subWeeks(prevDate.from, 1), { weekStartsOn: 1 }) : startOfWeek(addWeeks(prevDate.from, 1), { weekStartsOn: 1 });
            newTo = endOfWeek(newFrom, { weekStartsOn: 1 });
          } else {
            // เดือน -> ต้นเดือน-สิ้นเดือน
            newFrom = direction === "prev" ? startOfMonth(subMonths(prevDate.from, 1)) : startOfMonth(addMonths(prevDate.from, 1));
            newTo = endOfMonth(newFrom);
          }
          break;
        
        case "month": // ปี -> ต้นปี-สิ้นปี
          newFrom = direction === "prev" ? startOfYear(subYears(prevDate.from, 1)) : startOfYear(addYears(prevDate.from, 1));
